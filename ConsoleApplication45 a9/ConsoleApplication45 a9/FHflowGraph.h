...
FHflowGraph.h
Maximal flow
Modified version of the FHgraph.h file, algorithm is to
solve the maximal flow of a graph
HEADER COMMENTS PUT ABOVE THEIR RESPECTIVE CLASSES
*/
...
DATA
    ...
    Is a typedef for classes of sort
METHODS(mostly)
    static bool setNSortType(int whichType);
    PRE:SORT_BY_DATA, SORT_BY_DIST
    POS: sort type
...
METHODS
    ...
    PRE:object, object to be copied
    POS:copied object
DISP
All methods here display their titular table
...
public:
    ...
    PRE:a graph, and a weight
    POS:a weighted edge added to graph

    VertPtr addToVertexSet(const Object & object);
    PRE: takes a vertex
    POS: adds to master vertex table

DISP
Display methods, shows titular tables
...
CLEAR
    ...
    PRE: takes a vertex
    POS: sets vertex to starting vert

    bool setEndVert(const Object &x);
    PRE: takes a vertex
    POS: sets vertex to ending vert
ALGORITHMS
    CostType findMaxFlow();
    Client algorithm, computes max flow

private:
    VertPtr getVertexWithThisData(const Object & x);
    PRE data
    POS: vertex with that data

    bool establishNextFlowPath();
    based on dijkstra algorithm
    PRE: N/A
    POS:finds the vertex which will give maximum flow

    CostType getLimitingFlowOnResPath();
    PRE: N/A
    POS: finds the minimum of the residual graph

    bool adjustPathByCost(CostType cost);
    PRE:limiting flow(result of last method)
    POS:adjusts graph for maximum flow

    CostType getCostOfResEdge(VertPtr src, VertPtr dst);
    PRE 2 verticies
    POS cost of the connecting edge

    bool addCostToResEdge(VertPtr src, VertPtr dst, CostType cost);
    PRE verticies, cost of edge
    POS added to residual
